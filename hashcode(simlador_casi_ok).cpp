...
vector<vector<ii>> definirSchedule(vector<vector<int>> &tiemposLlegada, vector<vector<Edge*>> &adyList, int totalTime, int pointsPerCar, int nNodes, int nEdges, int particiones, bool doShuffle) {
	...
	forn(i,nNodes){
		...
		if(cantTotalAutosNodo != 0){
			...
			if(doShuffle) shuffle(primerAuto.begin(), primerAuto.end(), rng);
			else sort(primerAuto.begin(), primerAuto.end());
...
ll solve(vector<vector<ii>> &schedule, vector<Edge> &vEdges, vector<vector<int>> &pathOfCar, int totalTime, int pointsPerCar) {
	...
	//cout << nCars << '\n';
	forn(i,nCars) {
		//cout << "add 0 " << i << ' ' << pathOfCar[i][0] << '\n';
		pq.push({0,i,pathOfCar[i][0]});
	}
	ll ret = 0;
	int curt = 0;
	//cout << nCars << ' ' << nNodes << ' ' << sz(pq) << '\n';
	while(!pq.empty()) {
		Event ev = pq.top();
		pq.pop();
		assert(ev.time >= curt);
...
int main()
{
	...
	forn(i,500) { // definir el valor distintos tiempos de espera a probar
		int tiempoDemora = rng()%100;
		vector<vector<int>> tiemposLlegada = tiemposLlegadaPorSemaforo(adyList, pathOfCar, tiempoDemora, vEdges);
		vector<vector<ii>> schedule = definirSchedule(tiemposLlegada, adyList2, totalTime, pointsPerCar, nNodes, nEdges, 1+rng()%75, rng()%2);
